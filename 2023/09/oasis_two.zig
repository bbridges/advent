...
pub fn getPreviousNumber(line: []const u8, allocator: std.mem.Allocator) !i32 {
    var count: usize = 0;
    ...
    i -= curr_len - 1;
    var prev_number: i32 = 0;

    while (true) {
        prev_number = data[i] - prev_number;

        if (curr_len == count) return prev_number;
...
pub fn main() !void {
    ...
    while (input_it.next()) |line| {
        sum += try getPreviousNumber(line, allocator);
    }
