...
const Block = struct {
    ...
    fn move(self: Block, input: []u8, w: usize, h: usize, comptime dir: Dir) ?Block {
        if (self.stride < 4 and self.dir != dir) return null;

        switch (dir) {
            .Up => {
                if (self.dir == .Down or self.x == 0 or
                    self.dir == .Up and self.stride == 10) return null;
            },
            .Down => {
                if (self.dir == .Up or self.x == h - 1 or
                    self.dir == .Down and self.stride == 10) return null;
            },
            .Left => {
                if (self.dir == .Right or self.y == 0 or
                    self.dir == .Left and self.stride == 10) return null;
            },
            .Right => {
                if (self.dir == .Left or self.y == w - 1 or
                    self.dir == .Right and self.stride == 10) return null;
            },
        }
...
fn getMinHeatLoss(input: []u8, allocator: std.mem.Allocator) !u64 {
    ...
    var visited = try allocator.alloc([4 * 10]bool, input.len);
